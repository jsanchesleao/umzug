...
**Persistence.** All data lives in IndexedDB via Dexie (`src/data/db.ts`), as normalized stores — `apartments`, `timelineEvents`, `actions`, `photos`, `sketchPages`, plus the parallel Tasks stores `tasks`, `taskEvents`, `taskActions` — keyed by `id` with indexes on foreign keys, rather than nested documents. This is deliberate: it lets "all unresolved actions across every apartment, sorted by urgency desc then due date asc" run as a single indexed query (`getUnresolvedActions` in `src/data/actions.ts`, and its Tasks equivalent `getUnresolvedTaskActions` in `src/data/taskActions.ts`) instead of loading and flattening every apartment/task. The Dashboard merges both queries' results into one sorted list rather than sharing a table between the two entity types.

The Dexie schema is versioned in `db.ts` (`version(1)`, then `version(2)` with an `.upgrade()` migrating the old `rentCost` field into `coldRent`/`warmRent`, up through `version(5)` which added the Tasks tables as brand-new empty stores needing no `.upgrade()`). Future schema changes must add a new `.version(n)` block with its own upgrade function — never edit an existing version's `.stores()`/`.upgrade()` in place.

**Data access layer.** One file per entity in `src/data/` (`apartments.ts`, `timelineEvents.ts`, `actions.ts`, `photos.ts`, and their Tasks counterparts `tasks.ts`, `taskEvents.ts`, `taskActions.ts`) wraps all Dexie calls (CRUD + queries). Components should go through these functions rather than importing `db` and querying tables directly.

**Routing.** `/` (Dashboard), `/apartments` and `/apartments/:id` (Apartments list/detail), `/tasks` and `/tasks/:id` (Tasks list/detail), and `/documents` (Documents vault) — via `react-router-dom`'s `BrowserRouter` with `basename={import.meta.env.BASE_URL}` (`src/App.tsx`). This basename matters because `vite.config.ts` sets `base: "/umzug/"` on build, since the app is served from a GitHub Pages subpath.

**Tasks.** A second, independent case-file entity (`src/pages/Tasks.tsx`, `TaskDetail.tsx`) mirroring the Apartment/TimelineEvent/Action architecture one-for-one — its own `TaskStatus` enum (`ToDo`/`InProgress`/`Finished`/`Cancelled`, all transitions unrestricted and committed directly with no intermediate modal), its own Dexie tables, its own data-access files, and its own import/export pipeline (`src/data/taskImportExport.ts`, independent `ExportedTask` JSON shape) and P2P pairing channel (`src/components/TaskP2PSendModal.tsx`/`TaskP2PReceiveModal.tsx`, `umzugtask-` peer prefix, `?p2ptask=` link param — separate from both the apartments (`umzug-`/`?p2p=`) and documents (`umzugdoc-`/`?p2pdoc=`) channels so pairing sessions can't cross-connect). Several previously apartment-only components were generalized to be entity-agnostic and are now shared between Apartments and Tasks: `StatusBadge`, `StatusMenu`, `StatusFilterMenu`, `ColumnVisibilityMenu`, `CollapsibleSection`, `ActionRow`, `UnresolvedActionsPanel`, and `ImportCollisionDialog`.
