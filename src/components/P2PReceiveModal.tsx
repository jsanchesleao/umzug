...
type ReceiveState =
  ...
  | { phase: "connecting" }
  | { phase: "receiving" }
  | { phase: "collision"; apartments: ExportedApartment[]; collisionCount: number }
  | { phase: "done"; outcome: ImportOutcome }
  | { phase: "error"; message: string };

function P2PReceiveModal({ initialCode, onClose }: P2PReceiveModalProps) {
    ...
    createGuestPeer()
      .then((peer) => {
        ...
        conn.on("data", async (data) => {
          ...
          try {
            const apartments = parseImportPayload(message.data);
            const collidingIds = await detectCollisions(apartments);
            if (collidingIds.length > 0) {
              setState({ phase: "collision", apartments, collisionCount: collidingIds.length });
              return;
            }
  ...
  async function handleResolveCollisions(resolution: CollisionResolution) {
    ...
    try {
      const outcome = await importApartments(state.apartments, resolution);
      conn?.send({ type: "ack", outcome } satisfies P2PMessage);
      setState({ phase: "done", outcome });
    } catch (error) {
  ...
  function handleManualSubmit(event: React.FormEvent) {
    ...
    const code = parsePairingInput(manualInput);
    if (!code) {
  ...
  return (
    <Modal title="Receive apartments" onClose={onClose}>
      {state.phase === "scan" && (
        <>
      ...
      {state.phase === "manual" && (
        <form onSubmit={handleManualSubmit}>
          ...
          <div className="form-field">
            <label htmlFor="p2p-manual-code">Pairing code or link</label>
            <input
              id="p2p-manual-code"
              type="text"
              value={manualInput}
...
export default P2PReceiveModal;
