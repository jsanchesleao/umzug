...
import { describeOutcome, type ImportOutcome } from "../data/importExport";
import { buildTaskExport, buildAllTasksExport } from "../data/taskImportExport";
...
type Scope = { scope: "task"; taskId: string } | { scope: "all" };

interface TaskP2PSendModalProps {
  onClose: () => void;
}
...
function TaskP2PSendModal(props: TaskP2PSendModalProps & Scope) {
  ...
  useEffect(() => {
    ...
    async function acceptConnection(connection: DataConnection) {
      ...
      connection.on("open", async () => {
        ...
        try {
          const exported =
            scope === "task" ? await buildTaskExport(props.taskId) : await buildAllTasksExport();
  ...
  const title = scope === "task" ? "Send this task" : "Send all tasks";
      ...
      {(state.phase === "waiting" || state.phase === "sending") && (
        <>
          <div className="p2p-qr">
            <QRCodeSVG value={buildPairingLink(state.code, LINK_PARAM)} size={200} />
          </div>
          ...
          <div className="p2p-code-row">
            ...
            <button
              type="button"
              className="btn btn-sm"
              onClick={() => navigator.clipboard.writeText(buildPairingLink(state.code, LINK_PARAM))}
            >
              Copy link
            </button>
          </div>
...
export default TaskP2PSendModal;
